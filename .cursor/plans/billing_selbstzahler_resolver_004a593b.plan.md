...
**Final:** `bun run build` and `bun test` (add a **small unit test** for `resolveAcceptsSelfPayment` if the repo has a nearest pattern under `src/features/trips/lib/__tests__/` — optional but recommended for a pure function; if no test harness, rely on `bun test` passing for existing suite).
