...
- [`keys/trips.ts`](keys/trips.ts) — `tripKeys.detail(id)` for trip detail; `tripKeys.unplanned(filter)` / `tripKeys.unplannedRoot` for the dashboard **Offene Touren** list (`useUnplannedTrips`). After a save from that widget, **await** `invalidateQueries({ queryKey: tripKeys.unplannedRoot })` so row membership and `scheduled_at` in cache update before the success UX (see `pending-tours-widget.tsx`).
...
- [`keys/users.ts`](keys/users.ts) — `userKeys.list()` for **`/dashboard/users`** (`GET /api/users`); mutations should also invalidate `referenceKeys.drivers()` when `is_active` changes.
