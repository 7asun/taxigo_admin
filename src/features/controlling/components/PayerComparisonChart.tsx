...
import { aggregatePayers, formatEuro } from '../lib/controlling-utils';
...
export function PayerComparisonChart({
  breakdown,
  breakdownPrevious
}: PayerComparisonChartProps) {
  const currentPayers = useMemo(
    () => aggregatePayers(breakdown.data ?? []),
    [breakdown.data]
  );

  // payer_id is stable across periods; payer_name can be renamed in admin.
  const previousMap = useMemo(() => {
    const map = new Map<string, number>();
    for (const payer of aggregatePayers(breakdownPrevious.data ?? [])) {
      map.set(payer.payer_id, payer.revenue_net);
    }
    return map;
  ...
  const chartData = useMemo(
    () =>
      currentPayers.map((payer) => ({
        ...
        current: payer.revenue_net,
        previous: previousMap.get(payer.payer_id) ?? 0
      })),
    [currentPayers, previousMap]
  );
