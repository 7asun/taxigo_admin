-- Aggregated per-day rows for Schichtzettel list view (State B). See plan: get_shift_day_summaries.
-- Uses name / first_name / last_name on accounts (no full_name column).
...
AS $$
  SELECT
    ...
    COUNT(*) FILTER (WHERE p.accepts_self_payment = true)::bigint AS self_pay_count,
    COALESCE(
      SUM(COALESCE(t.manual_gross_price, t.gross_price))
        FILTER (WHERE p.accepts_self_payment = true),
      0
    ) AS self_pay_total,
    COUNT(*) FILTER (WHERE p.accepts_self_payment = false)::bigint AS invoice_count,
    COUNT(*) FILTER (WHERE p.accepts_self_payment IS NULL)::bigint AS unconfigured_count,
