...
**Status:** Phase 2 complete; **Phase 3A complete**; **Phase 3B complete — all scoped write paths migrated** (detail sheet + patch, reschedule dialog, Kanban time commit, dispatch inbox `handleAssign`; timeless hook query + widget copy; duplicate engine **skipped** by design). Phase 1 reference preserved below.  
**Note:** Post-3B gaps remain (bulk upload, upcoming trips window, print ranges, duplicate pairing UTC slice, `getTodaysTrips` device-local “today”, historical bad UTC rows, etc.) — see **Post-3B known gaps** at the end of this file.
...
### Post-3B known gaps (not shipped in 3B)

- **`getTodaysTrips` / driver day list:** device-local “today” vs Berlin business day (called out in `trips-date-filter.md`).
- **Bulk upload (`bulk-upload-dialog.tsx`):** CSV date+time still browser-local construction for inserts.
...
- **`duplicate-trips.ts` pairing / `recurring-exceptions.actions`:** UTC `split('T')[0]`-style risks called out in Section 1.4 — query/pairing, not the 3B write paths.
- **`use-pending-assignments` `todayStr` filter:** still `toISOString().slice(0,10)` for inbox “today” list scope — optional Berlin follow-up if product requires it.
