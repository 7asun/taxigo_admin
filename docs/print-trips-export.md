...
The user picks a **calendar day**, optionally an **Ab Uhrzeit** (`HH:mm`, default `00:00`), and clicks **ZIP generieren**. The client loads trips from Supabase where `scheduled_at` is **greater than or equal to** the lower bound for that calendar day and **less than or equal to** the end of that day (`endOfDay`). When **Ab Uhrzeit** is the default `00:00`, the lower bound is computed with **`startOfDay`** exactly as before (same `toISOString()` as the legacy full-day export). For any other time, the bound is the same local calendar day with that clock time (`setHours` on a copy of the selected `Date`, then `toISOString()`), not a UTC-midnight-parsed string. The upper bound is always end-of-day; there is no “Bis Uhrzeit” control. Rows with `status = 'cancelled'` are **excluded** in the query and again in a client-side filter (defense-in-depth), then the app builds a **ZIP** download:
