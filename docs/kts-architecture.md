...
**Last updated:** 2026-06-19 (PR4.2 Abrechnung tab + payment statuses)
...
| `kts_ruecklaufer_reason` | `text` (nullable) | **PR4.2:** Optional admin note from `mark_belegnummer_ruecklaufer`; cleared on resolve (reimport or manual `abgerechnet`). |
...
| `ruecklaufer` | Rückläufer | Returned for correction — blocks group payment until resolved |
| `bezahlt` | Bezahlt | Payment confirmed (PR4.2 Phase 1 manual; Phase 2 bank CSV deferred) — **terminal** |
...
| eligible trip | `abgerechnet` | RPC `apply_kts_invoice_import` (PR4/v4) — does **not** require `uebergeben`; re-stamps `ruecklaufer` rows on reimport |
| `abgerechnet` (group) | `bezahlt` | RPC `mark_belegnummer_bezahlt` (PR4.2) — all trips under Belegnummer; blocked if any `ruecklaufer` |
| `abgerechnet` (group) | `ruecklaufer` | RPC `mark_belegnummer_ruecklaufer` (PR4.2) |
| `ruecklaufer` (group) | `abgerechnet` | CSV reimport (`apply_kts_invoice_import` v4) or manual RPC `mark_belegnummer_abgerechnet` |
...
| `ungeprueft`, `korrekt`, `uebergeben`, `abgerechnet`, `ruecklaufer`, `bezahlt` | `false` |
| `NULL` | `false` |
...
Flow 2 (accountant invoice CSV): admin imports semicolon-delimited CSV from the accountant; system matches rows to trips (client-side in PR4.1), stamps invoice snapshot columns, sets `kts_status = abgerechnet`. **Amounts are invoiced, not paid** — Krankenkasse payment confirmation is Flow 3 (PR4.2: manual `bezahlt` / `ruecklaufer`; bank CSV reconciliation deferred to PR5).
...
**Badge:** `abgerechnet` — blue; `ruecklaufer` — orange; `bezahlt` — green (see `src/lib/kts-status.ts`).

**Next:** PR5 bank CSV reconciliation; PR4.3 (manual Unmatched linking, handover dropdown, import history).

### 3.8 KTS Abrechnung tab (PR4.2)

**Route:** `/dashboard/kts?view=abrechnung` — second tab on the existing KTS page. Legacy `view=list` (default) keeps the Bearbeitung queue unchanged.

**Grouping key:** `trips.kts_belegnummer` (invoice reference). One Belegnummer may span multiple trips (outbound + return). **No DB uniqueness** on Belegnummer — RPC exposes `import_count` / `has_multiple_imports` when the same Belegnummer appears in multiple import batches.

**Group status** (computed in SQL, not stored):

| Priority | Condition | `group_status` |
| -------- | --------- | -------------- |
| 1 | Any trip `ruecklaufer` | `ruecklaufer` |
| 2 | All trips `bezahlt` | `bezahlt` |
| else | — | `abgerechnet` |

**RPCs** (all `SECURITY DEFINER` with `current_user_is_admin()` + `p_company_id = current_user_company_id()`):

| RPC | Role |
| --- | ---- |
| `get_kts_abrechnung_groups` | Paginated grouped listing + import metadata |
| `get_kts_abrechnung_groups_count` | Pagination total — mirrors list filters |
| `get_kts_abrechnung_kpis` | Stat cards: Belege, invoiced €, paid €, open groups |
| `mark_belegnummer_bezahlt` | `abgerechnet` → `bezahlt` per Belegnummer; fails if any `ruecklaufer` |
| `mark_belegnummer_ruecklaufer` | `abgerechnet` → `ruecklaufer` per Belegnummer; persists optional reason on `trips.kts_ruecklaufer_reason` |
| `mark_belegnummer_abgerechnet` | Manual escape: `ruecklaufer` → `abgerechnet`; clears `kts_ruecklaufer_reason` |
| `apply_kts_invoice_import` v4 | Reimport path: accepts rows where `kts_belegnummer IS NULL OR kts_status = ruecklaufer` |

**Migrations:** `20260619190000`–`20260619190600` (includes `20260619190250_kts_ruecklaufer_reason.sql`).

**Import-date filter:** when `imported_from` / `imported_to` are set, groups with `imported_at IS NULL` (no linked import batch) are excluded — intentional, not a bug.

**UI layers:**

| Layer | Location |
| ----- | -------- |
| Tab switcher + KPI swap | `kts-header.tsx` |
| Listing RSC | `kts-abrechnung-listing-page.tsx` → group RPCs |
| Filters | `kts-abrechnung-filters-bar.tsx` — reuses URL `kts_status` for **group_status** filter; `imported_from` / `imported_to` (Abrechnung-only) |
| Table | `kts-abrechnung-table/` — separate from Bearbeitung `kts-table/` |
| Expand row | Client trip fetch + group actions (bezahlt / ruecklaufer / revert) |

**Deferred (not in PR4.2):** bank CSV bulk `bezahlt`, import history browser, mobile layout, optimistic UI updates.

### 3.5 KTS processing queue page (PR3.2)

**Route:** `/dashboard/kts` — top-level nav leaf (icon `post`, shortcut `k,s`). Tabs: **Bearbeitung** (default) + **Abrechnung** (`?view=abrechnung`, §3.8).
...
| **PR4.2** (shipped Phase 1) | `ruecklaufer` + `bezahlt` enum; Abrechnung tab; group RPCs; manual payment transitions; import v4 reimport — migrations `20260619190000`–`20260619190600`; bank CSV deferred to PR5 |
...
| **PR5** | Bank CSV reconciliation against external invoice numbers (bulk `mark_belegnummer_bezahlt`) |
...
**PR4.2 (2026-06):** Abrechnung tab (§3.8); `ruecklaufer` + `bezahlt` statuses; group/list/count/KPI RPCs; `mark_belegnummer_*` transitions; `apply_kts_invoice_import` v4 ruecklaufer reimport; migrations `20260619190000`–`20260619190600`.
...
*Changelog footer: 2026-06-19 — PR4.2: Abrechnung tab, ruecklaufer/bezahlt, group RPCs, import v4. 2026-06-10 — PR4.1: CSV import Dialog, matching utils, abgerechnet badge. PR4: §3.7 `kts_external_invoices`, `abgerechnet`, `apply_kts_invoice_import`. PR3.3: §3.6 handover. PR3.2: §3.5 queue. PR3.1: `kts_status`. PR1: §7.1 KTS write service.*
