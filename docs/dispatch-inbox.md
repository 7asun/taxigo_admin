...
2. **Responsive shell (mobile vs desktop)**:
   - **Below 768px** (`useIsNarrowScreen(768)`): the inbox opens as a **bottom `Drawer`** (Vaul) — full width, `max-h-[90dvh]`, scrollable list with safe-area padding at the bottom. `repositionInputs={false}` avoids awkward keyboard behaviour with time inputs. A screen-reader-only `DrawerTitle` labels the panel.
   - **768px and up**: the same content is shown in a **`Popover`** aligned to the trigger (`align="end"`), fixed width `420px` with `w-[min(100vw-1rem,420px)]` and `collisionPadding` so it stays on screen on small tablets.
   - The **Heute / Alle** tabs are **full width** under the title block on narrow viewports; from **`sm`** up they sit in the header row on the right (as before).
   - **`pending-assignment-item.tsx`**: passenger/time block stacks **vertically** below **`sm`**; **driver select** and **Zuordnen** are **full width** stacked on the phone, **row** layout from **`sm`** up (`min-w-0` on the row prevents horizontal overflow).
3. **Three-Tier Categorization**: The inbox splits pending trips into three distinct categories:
...
4. **Heute vs Alle Toggle**: Dispatchers can toggle between solely viewing trips relevant to "Heute" (Today) versus "Alle" (All Time) trips.
5. **Resilient Date Fallbacks**: Sourcing the correct date for an "Open Tour" requires calculating a fallback hierarchy since `scheduled_at` is empty. The inbox uses a unified calculation logic identically mirrored between the backend hook and the frontend row item:
...
- `pending-assignments-popover.tsx`: The UI shell for the inbox trigger and panel. Chooses **`Drawer`** vs **`Popover`** using `useIsNarrowScreen(768)` (see [`docs/mobile-ui.md`](mobile-ui.md)). Evaluates overall counts for the Bell badge and maps the three category lists into a scrollable region.
- `pending-assignment-item.tsx`: Represents a single row. Evaluates local state variations for the time input vs native original time to allow either driver assignment, time updates, or both concurrently. Responsive layout for narrow viewports (stacked controls) is described above.
