...
- **Stammdaten / Abrechnung / Hinweise:** Same values as on the open trip: `client_id`, `client_name`, `client_phone`, `is_wheelchair`, `payer_id`, `billing_variant_id`, `billing_calling_station`, `billing_betreuer`, `kts_document_applies`, `kts_source`, `kts_fehler`, `kts_fehler_beschreibung`, `no_invoice_required`, `no_invoice_source`, `notes` (merged from the notes textarea when that flow runs with dirty notes).
