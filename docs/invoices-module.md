...
- **Builder-only fields with no DB column** (documented reconstruction decisions):
  ...
  - **`resolved_rule` = null** (per-line rule / `billing_variant_id` / `client_id` are not stored, so it cannot be reconstructed for monthly invoices). KM-edit reprice in edit mode falls back to tax-rate-only adjustment; live reprice of edited lines is deferred to the save path.
