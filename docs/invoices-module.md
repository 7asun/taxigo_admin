...
**Cancelled trips (billing vs PDF):** The builder loads trips in two queries ([`invoice-line-items.api.ts`](../src/features/invoices/api/invoice-line-items.api.ts)): `fetchTripsForBuilder` excludes `status = cancelled` (constant `CANCELLED_STATUS`) so **`buildLineItemsFromTrips`**, **`calculateInvoiceTotals`**, and **`insertLineItems`** never see cancelled rows. A parallel **`fetchCancelledTripsForBuilder`** applies the same payer / period / billing variant / client filters and returns a narrow **`CancelledTripRow`** list for display only, including **`canceled_reason_notes`** (same snake_case column as Postgres **`trips.canceled_reason_notes`**) — selected only for the PDF appendix note line, never for billing. **`useInvoiceBuilder`** holds `cancelledTrips` separately from `lineItems`; **`show_cancelled_trips`** on the Vorlage/profile (default `false` on `pdf_column_override`, **`resolvePdfColumnProfile`**) gates whether any cancelled rows render. **`InvoicePdfDocument`** passes **`cancelledTrips`** only to appendix pages (never Haupttabelle): every billed appendix **`InvoicePdfAppendix`** gets **`cancelledTrips={[]}`** so stornierte Fahrten never repeat per Abrechnungsart page; when the gate is true and there are cancelled rows, a **final** appendix **`Page`** uses **`lineItems={[]}`**, **`groupLabel="Stornierte Fahrten"`**, **`cancelledTrips={scoped list}`**. Cell text comes from **`cancelledTripAppendixCell`** (`lib/cancelled-trip-appendix-cells.ts`), with **`getCanceledReasonNote(row)`** for the optional **`Stornierungsgrund:`** line under each row (plus the fixed **`Storniert – kein Rechnungsbetrag`** sub-line — see **`InvoicePdfAppendix`** **`renderCancelledSection()`**). **Re-downloading** an issued PDF still omits cancelled rows until **`TODO(issued-cancelled-rows)`** is solved (detail fetch refetches trips by period + payer).
